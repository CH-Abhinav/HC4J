...
pub extern "C" fn hc4j_gpu_download(id: u64, ptr_out: *mut f32, length: usize) -> i32 {
    ...
    let staging_buffer = eng.device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("HC4J Download Staging Buffer"),
        size: size_bytes,
    ...
    // Updated to v30 API polling syntax
    let _ = eng.device.poll(wgpu::PollType::Wait { submission_index: None, timeout: None });

    match receiver.recv() {
        Ok(Ok(())) => {
            {
                // Updated to handle the Result wrapper returned by get_mapped_range
